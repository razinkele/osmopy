...
def solve_tp(t_opt: float, fx: BioenFixed) -> float:
    ...
    # scipy's brentq stub is overloaded on full_output and collapses to a
    # `float | tuple[float, RootResults]` union; omitted here, so the runtime
    # value is always the scalar root.
    root = cast("float", brentq(lambda tp: _dg_dT_at(t_opt, tp, fx), lo, hi, xtol=1e-6))
    return float(root)
...
def bioen_param_lines(
    results, fx: BioenFixed, zlayer, sp_index, background_imax, notes, m0, background_beta=None
):
    ...
    background_beta: {file index: allometric exponent} for background predators in
    ``background_imax`` (default 0.8 for any index not given, matching
    ``osmose.engine.processes.bioen_predation.per_fish_ingestion_cap``'s hardcoded background
    beta and ``background_imax``'s own default -- the two must stay the same number or the
    "cap equals the standard cap at w_mean" property this file's Imax was solved for breaks).

    Deliberately narrower than the focal block for background predators (C3 Task 11): only
    ``species.beta`` and the Imax family are written. ``species.bioen.{assimilation,
    maint.*, mobilized.*}``, ``species.maturity.*`` and ``species.bioen.forage.*`` are
    FOCAL-length arrays in ``config.py`` (sized ``n_species``, not ``n_species+n_background``)
    -- the Python engine cannot read them at a background index, so authoring values for a
    background predator's T_p/c_m/maturity would be fabricated numbers with no code path
    behind them. Java's Gate-B inventory (spec Sec.4) wants the full block for every predator
    index; this file does not supply it, so it is not Java-loadable as authored (recorded in
    the README and, right above the background blocks, in this file's own comments)."""
    out = [
        ...
        (
            "# RMS len % (below) pins K and the growth-curve shape it implies; it is "
            "flat-to-inverted across t_opt/Linf errors (Imax/r absorb them) -- see README.md "
            "'What the RMS pin does and does not check' before treating a pass as validating "
            "t_opt or Linf."
        ),
    ]
    for res in results:
    ...
    if background_imax:
        out.append(
            "# NOT JAVA-LOADABLE AS AUTHORED: the background blocks below carry only "
            "species.beta and the Imax family. species.bioen.{assimilation,maint.*,"
            "mobilized.*}, species.maturity.* and species.bioen.forage.* are deliberately NOT "
            "authored for background predators (those arrays are focal-length in "
            "osmose/engine/config.py; the Python engine cannot read them at a background "
            "index). Java's Gate-B key inventory wants the full block for every predator "
            "index, focal and background -- a Java run of this overlay needs those keys "
            "added first (see README.md)."
        )
    for b_idx, imax in sorted(background_imax.items()):
        beta_b = 0.8 if background_beta is None else background_beta.get(b_idx, 0.8)
        out += [
            (
                f"# background predator sp{b_idx}: Imax already divided by n_dt_per_year -- Java's "
                f"bioen predation cap skips that division for background predators (ruling R1); "
                f"beta={_fmt(beta_b)} matches the Python engine's hardcoded background exponent "
                f"and is NOT read by the Python engine at runtime -- authored only so Java, which "
                f"has no default and errors on absence, can load this block"
            ),
            f"species.beta.sp{b_idx};{_fmt(beta_b)}",
