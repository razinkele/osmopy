...
- `latest_changelog_entry`: regex/scan for the first `^## \[(?!Unreleased)([^\]]+)\]` heading, capture
  the version and (optional) date, and the body up to the next `^## `. Tolerant of an empty or
  missing dated section (fallback as documented). Pure (operates on the passed text).
...
- **Version injection + auto-show script** in `app.py` (alongside the existing IIFE, before
  ...
  - a script that, on `DOMContentLoaded`, reads `localStorage.getItem("osmose_seen_changelog_version")`
    and — if it `!== window.OSMOSE_VERSION` — calls
    `bootstrap.Modal.getOrCreateInstance(document.getElementById("changelogModal")).show()`; and binds
    `changelogModal.addEventListener("hidden.bs.modal", ...)` to
    `localStorage.setItem("osmose_seen_changelog_version", window.OSMOSE_VERSION)`. Guards a missing
    element / undefined `bootstrap` / `localStorage` exceptions (private-mode → modal shows each load,
...
browser load:  localStorage["osmose_seen_changelog_version"] !== OSMOSE_VERSION  →  show #changelogModal
...
- `latest_changelog_entry`: no dated section → `[Unreleased]` body, else `"No release notes yet."`;
  never raises.
- Startup script: guards a missing `#changelogModal`, undefined `bootstrap`, and wraps `localStorage`
  access in `try/catch` (private-mode degrade = modal shows each load).
...
1. **Unit `tests/test_docs_content.py`** (pure):
   ...
   - `latest_changelog_entry(sample)` with a dated `## [0.13.0] - 2026-06-14` section → returns
     `version=="0.13.0"`, a `date`, and a `body` containing that section's bullets and **not** the
     next section's.
...
3. **e2e `tests/test_e2e_docs.py`**:
   - Fresh context (empty localStorage) → `#changelogModal` becomes visible automatically on load.
     Dismiss it, then `page.reload()` (same context preserves localStorage) → `#changelogModal` is
     **not** visible.
   - Click the header "About" link → `#aboutModal` visible → click the "Changelog" tab → it contains a
     known marker (e.g. `"[0.13.0]"`).
