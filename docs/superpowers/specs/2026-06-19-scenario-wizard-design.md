...
- `wizard_dest = reactive.Value[Path | None](None)` — the persistent demo dir for the current resolution
- `wizard_error = reactive.Value("")`

**Modal mechanism (pinned down):** a single `ui.modal_show(...)` whose body is a `@render.ui` that reads `wizard_step()` and renders ONLY the current step's controls (step 1 select + saved-source note; step 2 the three Basics inputs, pre-filled; step 3 name input), plus `wizard_error()` as an inline `<div class="text-danger">` when non-empty. Footer: Back / Next / Create buttons (Create shown only on step 3) as `@reactive.event` handlers that mutate `wizard_step`. The modal is removed only on Create-success or Cancel (`data-bs-dismiss`).
...
**Next-from-step-1 handler:** require a selected source else set `wizard_error` and stay. `(kind, name) = parse_source(value)`. If `"kind:name"` differs from `wizard_source_key`: create a fresh persistent dir `dest = Path(tempfile.mkdtemp(prefix="osmose_wizard_"))` (same `mkdtemp` call as `grid.py:809`'s `handle_load_example`, but DELIBERATELY without that handler's `atexit.register(shutil.rmtree, ...)` cleanup — `config_dir` must outlive the handler so map paths resolve at run time), store it in `wizard_dest`, `resolved = resolve_source(kind, name, scenarios_dir=state.scenarios_dir, dest_dir=dest)`, cache it in `wizard_source` + set `wizard_source_key`. (Re-selecting the same source reuses the cache; selecting a different one re-resolves.) Then prefill the step-2 inputs from `read_basics(resolved.config)` via `ui.update_numeric`/`ui.update_switch`, and advance `wizard_step=2`. Wrap resolution in try/except → on failure set `wizard_error`, stay on step 1.
...
- `resolve_source(kind="demo", "baltic", scenarios_dir=tmp, dest_dir=tmp2)` → config has `grid.nlon` etc., `config_dir` exists on disk, `case_map` non-empty, `parent is None`.
- `resolve_source(kind="scenario", ...)` against a `ScenarioManager` seeded in `tmp_path` → returns the stored config, `config_dir is None`, `parent == source`, `case_map` carried.
