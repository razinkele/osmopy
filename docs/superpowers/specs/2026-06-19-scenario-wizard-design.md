...
**Next-from-step-1 handler:** require a selected source else set `wizard_error` and stay. `(kind, name) = parse_source(value)`. If `"kind:name"` differs from `wizard_source_key`: for a demo source create a fresh dir `dest = Path(tempfile.mkdtemp(prefix="osmose_wizard_"))` and `atexit.register(shutil.rmtree, str(dest), True)` (same as `export_all_scenarios`) — `config_dir` must outlive the handler so map paths resolve at run time, but the dir is reclaimed at process exit. Then `resolved = resolve_source(kind, name, scenarios_dir=state.scenarios_dir, dest_dir=dest)` (scenario sources pass `dest_dir=None`), cache it in `wizard_source` + set `wizard_source_key`. (Re-selecting the same source reuses the cache; selecting a different one re-resolves.) Then advance `wizard_step=2` — the step-2 `@render.ui` reads the cached `wizard_source` and prefills each input at creation via `value=read_basics(resolved.config).<field>` (no post-creation `update_*` call, so no race). Wrap resolution in try/except → on failure set `wizard_error`, stay on step 1.
