...
**Next-from-step-1 handler:** require a selected source else set `wizard_error` and stay. `(kind, name) = parse_source(value)`. If `"kind:name"` differs from `wizard_source_key`: create a fresh persistent dir `dest = Path(tempfile.mkdtemp(prefix="osmose_wizard_"))` (same `mkdtemp` call as `handle_load_example` at `grid.py:819`, but DELIBERATELY without that handler's `atexit.register(shutil.rmtree, ...)` cleanup — `config_dir` must outlive the handler so map paths resolve at run time), store it in `wizard_dest`, `resolved = resolve_source(kind, name, scenarios_dir=state.scenarios_dir, dest_dir=dest)`, cache it in `wizard_source` + set `wizard_source_key`. (Re-selecting the same source reuses the cache; selecting a different one re-resolves.) Then prefill the step-2 inputs from `read_basics(resolved.config)` via `ui.update_numeric`/`ui.update_switch`, and advance `wizard_step=2`. Wrap resolution in try/except → on failure set `wizard_error`, stay on step 1.
