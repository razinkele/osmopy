...
2. Precomputes `prey_access_idx` / `pred_access_idx` / `access_matrix` for the call. Redundant if a test calls in a loop over cells, but test performance isn't a concern. Per-call recomputation also cleanly sidesteps the in-loop `access_matrix = _DUMMY_ACCESS` rebinding quirk at `predation.py:613-614` in the current batch `predation()` (the rebinding persists across cell iterations as a local-variable side effect; per-call recomputation has no such cross-call state).
3. Ensures `feeding_stage` is current on `state`. Implementation: `state.feeding_stage[:] = compute_feeding_stages(state, config)` — an in-place numpy array copy into the existing buffer. This works even though `SchoolState` is `@dataclass(frozen=True)` (`osmose/engine/state.py:30`) because frozen dataclasses block attribute assignment but allow mutation of mutable attribute values, and `feeding_stage` is always pre-allocated to `np.zeros(n)` by `SchoolState.create`. Always recomputes, matching batch `predation()` semantics — no "is it already populated" check, because the `np.zeros` initial value is indistinguishable from valid "all stage 0" data and guessing would mask bugs.
...
5. If `resources is not None and resources.n_resources > 0`, invokes `_predation_on_resources` for the same cell coordinates. Note: `_predation_on_resources` draws `rng.permutation(n_local)` internally (`predation.py:394`). Sequential `predation_for_cell` calls on a shared `rng` with mixed `resources=None` / `resources=obj` will produce a different RNG draw sequence than batch `predation()` would have (batch either threads resource predation through every cell or none). No current test mixes resource modes across calls; migrated tests must not introduce the pattern.
...
**Fields mutated in place on `state`:** the new API modifies exactly four fields via direct numpy array writes (verified against `_predation_in_cell_python:351-363`):
- `state.abundance[...]` — set via `= max(0.0, ...)` (absolute assignment)
- `state.pred_success_rate[...]` — **accumulating** via `+=`
- `state.preyed_biomass[...]` — **accumulating** via `+=`
- `state.feeding_stage[...]` — fresh overwrite via `[:] =` (step 3 above)

The two accumulating fields are the critical porting hazard. Batch `predation()` creates fresh working copies before delegating (`predation.py:541-548`) so every call's returned state starts with the CALLER's initial `pred_success_rate` / `preyed_biomass` values, not the previous call's accumulation. Under `predation_for_cell`'s in-place contract, any test that calls it twice in sequence on the SAME `state` will see accumulated values on the second call. Class-G migration must check each site for this pattern and, if the test's assertion relied on fresh-copy semantics, either (a) zero the relevant fields between calls explicitly, or (b) construct a fresh `state` per call. This is a separate audit column from the identity/chaining concern in class G.
...
| `pred_success_rate` / `preyed_biomass` accumulation across calls | Batch `predation()` copies these fields before mutating so the returned state is per-call isolated. In-place `predation_for_cell` instead accumulates via `+=` into the caller's state. Tests that called `predation()` twice in sequence sharing one `state` and asserted on single-call values will silently see 2× on the second call. | Audit pass has a dedicated checklist column beyond A-G: "does this test call `predation()` more than once on the same `state` reference?" — if yes, migration either zeros `state.pred_success_rate[:] = 0` / `state.preyed_biomass[:] = 0` between calls or constructs a fresh state per call. The `abundance` field is also mutated in place but uses absolute assignment (not `+=`), so double-call semantics there are different (second call reduces further from the first call's already-reduced values, which is usually what the test wanted anyway). |
