...
- `MovementSnapshot` (a plain `@dataclass`, not frozen) gains `stage: NDArray[np.int8]` — one value
  per included school: **0 = egg/larva, 1 = juvenile, 2 = adult**.
...
- **Full call-graph ripple (5 functions + 1 call site) — all must change in lockstep, else the
  stage filter silently drops on one path:** `_filter_mask`, `_points_to_rows`, `heatmap_layer_from_points`,
  `dots_layer_from_points`, and `choose_live_layer` each gain a `stage_filter` parameter; the call site
  `_render_live_map` (`run.py:623`) passes it. **Pass `stage_filter` as a keyword arg** (e.g.
  `choose_live_layer(snap, species_filter, mode, stage_filter=...)`) to avoid breaking the existing
  positional call.
- **The dots-vs-heatmap fallback count must use the COMPOSED mask:** `choose_live_layer` decides the
  fallback from `_filter_mask(...).sum()` (`render.py:127`) — after this change that count must reflect
  `species_mask & stage_mask`, NOT species-only (else "Adult" on a >1500-school species wrongly falls
  back to heatmap). Threading `stage_filter` through `_filter_mask` at that call site achieves this.

### 2.3 UI + server — `ui/pages/run.py`
- **Species fix (the bug — half of it):** populate `live_movement_species` choices from the **config
  species names** as soon as a config is loaded (a reactive on `state.config` reading the focal
  `species.name.sp*` keys), i.e. `{"__all__": "All species", <name>: <name>, ...}`.
  `_populate_live_species` (from live frames) stays as a refresh but is no longer the only source — so
  the dropdown is **populated + clickable immediately**.
- **The OTHER half (must be owned, not hidden):** `_render_live_map` returns early when
  `_live_snapshot is None` (`run.py:621`), and the snapshot is `None` until a stream produces frames
  (it is reset to `None` at run start and retained after a run completes). So **selecting a species/stage
  only changes the map when a snapshot exists — during a live stream or from the retained final frame
  after a run.** On a freshly-loaded page (no run yet) the map is empty and the selector has no visible
  target. This is expected, not a bug — but to stop users concluding "selection is broken", **add an
  empty-state hint** in `live_movement_status` when `_live_snapshot is None` (e.g. "Run the model to
  populate the live map; then filter by species / stage."). This is the minimal honest completion of
  the fix; a pre-run config-derived preview is explicitly out of scope (§6).
...
- **Horizontal layout:** wrap Mode + Species + Stage in `ui.layout_columns(..., col_widths=[3, 5, 4])`
  (narrow radio, wider selects for long species names; not equal thirds) so the three controls sit
  side-by-side. Mode stays `input_radio_buttons` inline; Species/Stage are `input_select`.
...
config load → species choices populated (immediately, dropdown usable). engine frame →
`build_snapshot` (now with `stage`) → `_live_snapshot` → `_render_live_map` reads (mode, species, stage)
→ `choose_live_layer` applies `species_mask & stage_mask` → deck.gl layer. **No-snapshot path:** with
`_live_snapshot is None` (no run yet), `_render_live_map` early-returns — the selectors are populated but
the map stays empty + shows the empty-state hint; filtering takes effect once a stream (or retained final
frame) provides a snapshot and the card is expanded.
...
- **Unit (`live_movement_render.py`):** `_filter_mask` with `stage_filter=1` returns only juvenile
  schools; species+stage compose (intersection); `None` filters select all. Plus a `choose_live_layer`
  test asserting the **dots-vs-heatmap fallback count uses the composed species+stage mask** (a >dots_max
  species with few adults + `stage_filter=2` stays in dots mode, not heatmap).
