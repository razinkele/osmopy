...
**Shared-`RV_ref` caveat (load-bearing for the A/B):** the climatology and interannual fields do NOT auto-share a reference — each computes `RV_ref` on its own array, and averaging (climatology) vs concatenating (interannual) changes the RV>0 cell set, so the two attrs differ. To keep the A/B a clean isolation of *temporal structure*, the **Phase-3 harness forces one shared `RV_ref` on both arms** via the `reproduction.rv.spatial.ref` config key (`_load_rv_spatial` prefers a positive `ref` over the field attr). Use the interannual field's `RV_ref` as the shared value. Note (Jensen): because the gate `clip(RV/RV_ref, 0, 1)` is nonlinear, the two arms do NOT have *identical* time-mean suppression even with a shared `RV_ref` — the interannual arm's variance rectifies through the clip. That residual is itself part of the interannual signal being tested; the shared `RV_ref` removes the *reference* confound, which is the point. The harness compares against the deployed climatology, so both arms carry the same seasonal-mean field.
