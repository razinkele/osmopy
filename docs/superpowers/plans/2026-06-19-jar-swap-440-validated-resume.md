...
**Status: DEFERRED (resume-ready, 2026-06-19).** The jar swap was paused after the A/B gate FAILED: the 4.4.0 jar cannot LOAD OSMOPY's Java-runnable configs (`NETCDF_BIOMASS resource forcing ... species.biomass.varname.spN not found`) — a newly-REQUIRED 4.4.0 config-key class the original audit missed. PR-A (value-migration layer, `c9454d9`/#79) is merged and inert. **Resume trigger:** a concrete need for a specific 4.4.x Java feature. This plan is the detailed, validation-centered guide for that resume.

> **⚠️ RETARGET TO v4.4.1 (deep-review finding, 2026-06-21 — source-verified).** Part of the gate failure was a **genuine v4.4.0 engine bug**, not an OSMOPY-config defect: v4.4.0's `ResourceForcing.java` *gates* the NetCDF branch on the legacy `species.file.spN` but *reads* `species.biomass.file.spN` — a gate/read mismatch, so **v4.4.0 cannot load even its own bundled EEC NetCDF config**. **v4.4.1** (released 2026-06-18T08:56Z, the day BEFORE this deferral, now "Latest") **explicitly fixes this** (release notes: *"correct the parameter names in the initialization of resource forcings — species.biomass.file.sp instead of species.file.sp"*; verified in v4.4.1 source — the branch now gates on `species.biomass.file.spN`, the same key it reads). **Resume against v4.4.1, NOT v4.4.0.** This LOWERS resume cost (the file-key gate fix is upstream-done) but adds NO new value (no driving feature), so the deferral still holds. Caveats that PERSIST in v4.4.1: `species.biomass.varname.spN` is still hard-required (no default) → the varname migration is still needed; BoB's legacy `ltl.*` file-key synthesis is still needed; v4.4.1's two new params (`simulation.kill.if.no.school.enabled`, `species.is.enabled.spX`) are OPTIONAL (canFind-guarded) → no new required keys. The PR-A version gate (`aliases.py:189`, suffix-tolerant `>=4.4.0`) already treats `"4.4.1"` as ≥4.4.0, so PR-A's value layer fires unchanged. **Phase 0 must re-run the empirical load loop against the v4.4.1 jar and re-anchor the line-cited source claims below to the v4.4.1 tag.**
...
**Tech Stack:** Python 3.12, pytest, OSMOSE Java jar (subprocess), xarray (NetCDF), the ICES MCP tools + `osmose/validation/ices.py`. Run with `.venv/bin/python`. Java 21 present; **jar re-fetch (v4.4.1 — see retarget note above): `gh release download v4.4.1 -R osmose-model/osmose -p '*jar-with-dependencies*' -D osmose-java/`** (asset `osmose-4.4.1-jar-with-dependencies.jar`, 24,408,404 bytes; **capture the sha256 at download time** — `sha256sum osmose-java/osmose-4.4.1-*.jar` — GitHub does not publish it). (Superseded: v4.4.0 `gh release download v4.4.0 …` sha256 `70e4c01f…` — known-buggy resource forcing, do NOT bundle.)
...
**SCOPE CORRECTED (deep-review, verified against `data/examples` + the NetCDF):** BoB is LESS work than originally framed. `data/examples/osm_all-parameters.csv:81-99` ALREADY carries native `species.type.sp8-13=resource` + `species.name.sp8-13` (SmallPhyto…LargeDetritus) + `simulation.nresource;6`, and those names are **1:1 with the BoB NetCDF data vars** (`roms_n2p2z2d2_biscay.nc`) — so `varname=species.name.spN` works for BoB exactly like EEC, and NO `ltl.*→species.*` *identity* normalization is needed (4.4.0 derives `nResource` from `species.type.sp*`, not from `ltl.name.rscN`; the legacy `ltl.name/tl/size.rscN` keys are inert dead-weight 4.4.0 never reads). The reader does NOT normalize `ltl.*` (verified: `demo.py` `_MIGRATION_CHAIN` only does `plankton.*→resource.*`), but it doesn't need to.
- [ ] The ONE genuine BoB-extra step over EEC: synthesize a per-resource `species.file.spN` (the line-216 `isNull` gate key) + `species.biomass.file.spN` from the single global `ltl.netcdf.file` (`osm_param-ltl.csv:7`), for each resource sp-index (`sp = nspecies(8)+N`); then the same `species.biomass.{mode,varname,nsteps.year}.spN` step as Task 1.1 (varname = the already-present `species.name.spN`, cross-checked against the NetCDF data vars). TDD + the full-year load+run smoke on `data/examples`. Commit.
- [ ] **⚠️ BoB time-axis BLOCKER (verified):** the BoB NetCDF has **365 daily steps**, but 4.4.0 `ForcingFile.update()` requires `ndt/ncPerYear` to be a positive integer (i.e. `species.biomass.nsteps.year` must DIVIDE `ndt=24`). 365 neither divides 24 nor matches the EEC 24-step scheme, and the init-time guard that would catch this is COMMENTED OUT upstream — so a wrong value passes init then mis-maps/corrupts the forcing in `update()`. Do NOT copy `nsteps.year` from `ndtperyear` for BoB. The BoB daily-ROMS forcing is likely **incompatible with 4.4.x `NETCDF_BIOMASS` as-is** and may need re-sampling to a 24-step (ndt-dividing) axis or a different resource mode. The mandatory ≥1-full-year load+RUN smoke is the gate that surfaces this. (This is the real reason BoB is harder — not the key normalization.)
...
- [ ] Read `data/baltic` → migrate to 4.4.0 keys (`canonicalize_config` / the post-cutover default write) → write → re-read → run the Python engine. **This is a same-engine, same-RNG-stream round-trip, so the correct assertion is BIT-EXACT, not statistical:** set `simulation.rng.fixed=true` + an explicit fixed seed (e.g. 0), do ONE deterministic Python run pre-cutover (4.3.3 write/read path) and ONE post-cutover (4.4.0-target write → re-read), and assert per-species outputs are **bit-identical** (`np.array_equal` / tolerance literally 0; single run each, NO ensemble). Any nonzero diff is a config round-trip migration defect, NOT RNG noise — reserve TOST/equivalence/KS language strictly for the cross-RNG Phase 2 Java comparisons. This jar-free gate is runnable TODAY and closes the one channel by which the swap can affect the (otherwise Python-only, ICES/HOLAS-3-validated) Baltic config. Commit.
...
- **Reconsider-before-resuming:** the swap remains low-present-value (Java path low-centrality for validation/calibration — Python primary, Baltic/bioen/`nbackground>0` Java-blocked; *though* Java is still the UI **default** engine at `ui/state.py:64`/`app.py:481`, so a fresh user lands on the stale 4.3.3 jar — a cheap orthogonal fix is to flip the UI default to Python, unrelated to the swap). No new feature is exposed. Resume only when a concrete 4.4.x Java feature justifies it — then this plan delivers it with the strongest validation actually achievable, honestly bounded.
- **Smallest de-risking step (deep-review recommendation):** at resume, land **Phase 0 + Phase 1 ONLY against the v4.4.1 jar** — run the empirical fix-forward load loop until `data/eec_full` (and ideally `data/examples`) LOAD + run a short smoke, **without committing to the PR-B cutover** (keep `target_version` default `4.3.3`, keep the 4.3.3 jar for rollback). A few hours converts the entire required-key uncertainty into a verified yes/no, fully reversible, before any decision to flip the default.
- **Wording fix (DIAG-4):** the original plan did not *mis-state* the resource-forcing keys — it **never enumerated** them (its audit was dynamics-only); that omission is the audit-completeness lesson Phase 0 addresses.
