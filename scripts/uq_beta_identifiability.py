...
for _sp, idx, b in BETAS:
    fps.append(
        FreeParameter(f"stock.recruitment.shape.sp{idx}", b - HALF, b + HALF, Transform.LINEAR)
    )
    beta_star.append(b)
...
def main():
    ...
    with make_engine_evaluator(
        fps, BASE, SPECIES, enable_ssb=True, nyear=NYEAR, n_workers=NW
    ) as ev:
        ...
        print(
            "targets (arith-mean biomass at beta*):",
            {t.species: round(t.target, 1) for t in targets},
            flush=True,
        )
        res = run_surrogate_bayes(
            ev,
            fps,
            targets,
            n_seeds=NSEED,
            ...
            n_max=N0,
            seed=0,
            gate_fn=passthrough_gate,
            k_by_type={"biomass": 1.0},
            include_predictive=True,
        )
        holdout = run_design(
            ev, fps, KEYS, n_points=N_HOLD, n_seeds=NSEED, seed=99, seed_offset=7_000_000
        )
    ...
    with open(SAVE, "wb") as fh:
        pickle.dump(
            {
                "design": res.design,
                "sampler_result": res.sampler_result,
    ...
    for k, r in natural_gate.items():
        print(
            f"  {k:<22} real_pass={r.passed!s:<5} cov={r.coverage:.3f} mssr={r.mssr:.3f} "
    ...
    if res.sampler_result is not None:
        ...
        print(
            f"  {'beta param':<34}{'beta*':>8}{'post_mean':>11}{'post_sd':>9}{'conc':>7}{'center':>8}  verdict"
        )
        for j, fp in enumerate(fps):
            ...
            print(
                f"  {fp.key:<34}{beta_star[j]:>8.3f}{pm:>11.3f}{ps:>9.3f}{conc:>7.2f}{center:>8.2f}  {verdict}"
            )
    ...
    for k in KEYS:
        print(
            f"  {k:<22} coverage={hcov[k]:.3f}  (n_valid={int((~np.isnan(holdout.Y[k])).sum())}/{N_HOLD})"
        )
