...
def _classify(lat: float, lon: float) -> str:
    ...
    if lat >= 60.4:
        return "BothnianSea"
    if lat >= 59.0 and lon >= 22.9:
        return "GulfOfFinland"
    if lat >= 58.5 and lon < 21.0:
        return "NBalticProper"
    if lat >= 57.0 and 21.0 <= lon <= 24.5:
        return "GulfOfRiga"
    if lat >= 58.0 and lon < 19.5:
        return "WGotland"
    if lat >= 56.8 and 18.0 <= lon <= 21.5:
        return "EGotland"
    if lat >= 54.8 and lon >= 16.0:
        return "BornholmGdansk"
    if lat >= 54.8 and 13.0 <= lon < 16.0:
        return "Arkona"
    if lat >= 54.8 and lon < 13.0:
        return "BeltSoundKiel"
    return "Mecklenburg"
...
def main() -> None:
    ...
    print(
        f"  GreySeal per-timestep total: {seal_data[0].sum():.1f} t (expected {SEAL_STANDING_T:.1f})"
    )
    print(
        f"  Cormorant per-timestep total: {cormorant_data[0].sum():.1f} t (expected {CORMORANT_STANDING_T:.1f})"
    )
    grid.close()
