...
def run_arm(raw, cfg_dir, overlay, n_sp, ndt):
    ...
    acc = {k: np.zeros(n_sp) for k in ("w", "neg", "zero", "gonad", "n")}
    ...
    def observer(step, state, grid, config, map_sets):
        ...
        length = np.asarray(state.length, dtype=np.float64)
        age_y = np.asarray(state.age_dt, dtype=np.float64) / ndt
        gon = np.asarray(state.gonad_weight, dtype=np.float64)
        en = np.asarray(state.e_net, dtype=np.float64)
        is_egg = np.asarray(state.is_egg)
        m0 = getattr(config, "bioen_m0", None)
        m1 = getattr(config, "bioen_m1", None)
        for s in range(n_sp):
            sel = (sp == s) & (abd > 0) & ~is_egg
            if not sel.any():
                continue
            thr = 0.0 if m0 is None else float(m0[s]) + (
                0.0 if m1 is None else float(m1[s]) * age_y[sel]
            )
            mature = sel.copy()
            mature[sel] = length[sel] >= thr
            if not mature.any():
                continue
            a = abd[mature]
            acc["w"][s] += a.sum()
            acc["neg"][s] += a[en[mature] < 0.0].sum()
            acc["zero"][s] += a[gon[mature] == 0.0].sum()
            acc["gonad"][s] += (gon[mature] * a).sum()
            acc["n"][s] += 1
    ...
    try:
        simulate(ec, grid, rng, movement_rngs=mv, mortality_rngs=mo,
...
def main() -> int:
    ...
    print("=" * 96)
    print(f"{'species':<13}{'arm':<11}{'frac e_net<0':>14}{'frac gonad==0':>16}{'mean gonad t/fish':>20}")
    for s in FOCUS:
        i = names.index(s)
    ...
    print(f"{'species':<13}{'SSB bioen/base':>16}{'eggsLin bioen/base':>20}{'eggs per SSB ratio':>20}")
    for s in FOCUS:
        i = names.index(s)
    ...
    zc = {s: res["bioen"]["acc"]["zero"][names.index(s)]
          / max(res["bioen"]["acc"]["w"][names.index(s)], 1e-30) for s in FOCUS}
    ...
    if not e1:
        ...
        print("  substep destroys accumulated reproductive investment on every negative-E_net step,")
