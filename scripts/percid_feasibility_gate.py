...
CONTRASTS = {
    ...
    "cormorant-only": {
        "species.biomass.multiplier.sp15": "3.0",
        "predation.ingestion.rate.max.sp15": "80.0",
        "fisheries.rate.base.fsh4": BASE_PERCH_F,
        "fisheries.rate.base.fsh5": BASE_PP_F,
    },
}
...
for sp, (lo, hi) in ENVELOPE.items():
    vals = {
        k: (v.get(sp, float("nan")) if v is not None else float("nan")) for k, v in results.items()
    }
    print(
        f"{sp:12s} {vals['both']:12.0f} {vals['F-only']:12.0f} {vals['cormorant-only']:12.0f} "
        f"{f'[{lo},{hi}]':>18s}"
    )
...
if b is not None:
    ...
    c_moves = (
        results["cormorant-only"] is not None
        and results["cormorant-only"]["perch"] < b["perch"] * 3
    )
