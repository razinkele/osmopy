...
def build_config(src_dir: Path, bundle_dir: Path) -> Path:
    ...
    drop_exact = {
        ...
        "fisheries.catchability.file",
        "fisheries.discards.file",
    }
    for k in list(raw):
        if (
            k in drop_exact
            or k.startswith("movement.")
            and ".map" in k
            or k.startswith(("fisheries.movement.", "osmose.configuration."))
