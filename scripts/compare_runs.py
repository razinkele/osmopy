...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--metric", type=str, default="biomass", choices=["biomass", "yield", "abundance"]
    )
    ...
    deltas = az.run_delta(
        baseline, variant, metric=args.metric, window_years=args.window_years, top_n=args.top_n
    )
    ...
    if args.plot:
        ...
        plotting.make_run_delta_chart(deltas, metric=args.metric).write_html(
            f"{args.plot}_delta.html"
        )
    return 0
