...
def _call_c_bench(
    args: list, ffi: Any, lib: Any, snaps: tuple, n_iter: int, reset_only: bool
) -> None:
    ...
    _ia_arr, ia_ptr = _f64(a["inst_abd"])
    ...
    _psr_arr, psr_ptr = _f64(a["pred_success_rate"])
    _pb_arr, pb_ptr = _f64(a["preyed_biomass"])
    ...
    _tlws_arr, tlws_ptr = _f64(a["tl_weighted_sum"])
...
def boundary_probe(
    ...
    _ci_a, ci_p = _i32(a["cell_indices"])
    _ia_a, ia_p = _f64(a["inst_abd"])
    _si_a, si_p = _i32(a["species_id"])
    _le_a, le_p = _f64(a["length"])
    _we_a, we_p = _f64(a["weight"])
    _ag_a, ag_p = _i32(a["age_dt"])
    _ff_a, ff_p = _i32(a["first_feeding_age_dt"])
    _fs_a, fs_p = _i32(a["feeding_stage"])
    _ps_a, ps_p = _f64(a["pred_success_rate"])
    _pb_a, pb_p = _f64(a["preyed_biomass"])
    _tl_a, tl_p = _f64(a["trophic_level"])
    sx_a = np.ascontiguousarray(a["size_ratio_max"], dtype=np.float64)
    _ir_a, ir_p = _f64(a["ingestion_rate"])
    _fr_a, fr_p = _i32(a["fr_shape"])
    _fh_a, fh_p = _f64(a["fr_halfsat"])
    _pa_a, pa_p = _i32(a["prey_access_idx"])
    _pd_a, pd_p = _i32(a["pred_access_idx"])
    _rm_a, rm_p = _f64(a["rsc_size_min"])
    _rx_a, rx_p = _f64(a["rsc_size_max"])
    _rt_a, rt_p = _f64(a["rsc_tl"])
    _ra_a, ra_p = _i32(a["rsc_access_rows"])
    _tw_a, tw_p = _f64(a["tl_weighted_sum"])
    _pt_a, pt_p = _i32(a["prey_type_buf"])
    _pi_a, pi_p = _i32(a["prey_id_buf"])
    _pe_a, pe_p = _f64(a["prey_eligible_buf"])
    _er_a, er_p = _f64(a["egg_retained"])
...
def run_all(
    ...
    for label, cell in sel.items():
        r = bench_cell(arrays, meta, cell, variant=variant, n_iter=n_iter, n_samples=n_samples)
        results[label] = r
        w = float(r["n_local"])
        total_weight += w
    ...
    results["boundary"] = boundary_probe(arrays, meta, sel["small"], variant=variant)
