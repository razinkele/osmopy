...
def test_validation_loads_phase12_without_phase1_phase2_stacking(tmp_path, monkeypatch):
    """Joint phase 12 validation must not silently use stale phase 1+2 outputs."""
    import scripts.calibrate_baltic as cal

    results_dir = tmp_path / "results"
    results_dir.mkdir()
    (results_dir / "phase1_results.json").write_text(
        json.dumps({"parameters": {"mortality.additional.rate.sp0": 0.2}})
    )
    (results_dir / "phase2_results.json").write_text(
        json.dumps({"parameters": {"fisheries.rate.base.fsh0": 0.08}})
    )
    (results_dir / "phase12_results.json").write_text(
        json.dumps({"parameters": {"stock.recruitment.ssbhalf.sp5": 12345.0}})
    )
    monkeypatch.setattr(cal, "RESULTS_DIR", results_dir)

    overrides = cal._load_phase_overrides_for_validation("12")

    assert overrides == {"stock.recruitment.ssbhalf.sp5": "12345.0"}


def test_validation_stacks_phase2_on_phase1(tmp_path, monkeypatch):
    """Sequential phase 2 validation still inherits phase 1 mortality params."""
    import scripts.calibrate_baltic as cal
    ...
    overrides = cal._load_phase_overrides_for_validation("2")

    assert overrides == {
        "mortality.additional.rate.sp0": "0.2",
        "fisheries.rate.base.fsh0": "0.08",
    }


def test_report_warns_when_phase12_json_is_legacy(capsys):
    """A 24-param legacy phase 12 JSON should be visibly flagged in reports."""
    from scripts.report_calibration import warn_if_result_schema_stale
    ...
    warn_if_result_schema_stale("12", data)

    out = capsys.readouterr().out
    assert "WARNING" in out
    assert "Expected 27 params" in out
    assert "stock.recruitment.ssbhalf.sp5" in out
