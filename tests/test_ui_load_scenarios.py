...
def test_all_demos_produce_unique_configs(tmp_path):
    ...
    # discriminator, but some DRY overlays share baltic's species roster by design: baltic_a2
    # differs in depletable plankton + calibrated mortality, and baltic_depensation differs only
    # in the recruitment depensation gate (it inherits baltic's roster + mortality). So also treat
    # those overlay-specific keys as distinguishers.
    ...
    for i, a in enumerate(demo_list):
        for b in demo_list[i + 1 :]:
            distinguishers = (
                ...
                "reproduction.depensation.gate.enabled",
            )
