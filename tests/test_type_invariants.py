...
def test_resource_species_size_order():
    ...
    with pytest.raises(ValueError, match="size_min"):
        ResourceSpeciesInfo(
            name="test", size_min=10.0, size_max=5.0, trophic_level=2.0, accessibility=0.5
        )
...
def test_resource_species_accessibility_bounds():
    ...
    with pytest.raises(ValueError, match="accessibility"):
        ResourceSpeciesInfo(
            name="test", size_min=1.0, size_max=10.0, trophic_level=2.0, accessibility=1.5
        )
...
def test_background_species_length_count_mismatch():
    ...
    with pytest.raises(ValueError, match="n_class"):
        BackgroundSpeciesInfo(
            ...
            n_class=3,
            lengths=[5.0, 10.0],
            trophic_levels=[2.0, 2.5],  # only 2, not 3
            ages_dt=[0, 12],
            condition_factor=0.01,
            allometric_power=3.0,
            size_ratio_min=[0.5],
